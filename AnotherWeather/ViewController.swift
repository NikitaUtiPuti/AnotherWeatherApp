...
//        let randomColor = UIColor.init(red: .random(in: 0...1), green: .random(in: 0...1), blue: .random(in: 0...1), alpha: 1)
        ...
        self.weatherIcon.image = UIImage(named: weather.systemIconNameString)
            ...
            self.feelTempLabel.isHidden = false
            self.cityNameLabel.isHidden = false
            self.currentData.isHidden = false
            self.feelText.isHidden = false
            self.searchButtonOut.isHidden = false
            self.dataDetailButtonOut.isHidden = false
            self.forecastButtonOut.isHidden = false
        }
    }
...
//Интерфейс третьего экрана (смена дня и ночи на бэкграунде, цвета иконок и ШРИФТЫ)
...
//Проверка ошибок получения данных отедльным классом! (Интерфейс скрыт пока не получены данные) ! +-+-+-+-
...
//Менеджер форматированиия интерфейса третьего экрана (смена дня и ночи, настройка прозрачности тэйблвью, смена цветов текста в зависимости от бэка) (рефакторинг совместно с Day details) ++++++++++++
